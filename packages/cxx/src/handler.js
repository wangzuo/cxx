...
export default ({ routes, schema }, options) => async (req, res, next) => {
  ...
  if (Object.keys(matchLocation.query).length) {
    // todo
    const variables = matchLocation.variables
      ? matchLocation.variables(matchLocation)
      : {};
    const operation = createOperationSelector(
      getOperation(matchLocation.query),
      variables
    );
      ...
      onCompleted: () => {
        const markup = renderToString(
          <QueryRenderer
            ...
            query={matchLocation.query}
            variables={variables}
            render={result =>
              React.createElement(matchLocation.default, result)}
          />
        );
    ...
    const markup = renderToString(<matchLocation.default />);
